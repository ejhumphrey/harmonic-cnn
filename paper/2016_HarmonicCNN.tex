...
Note the octave-local model can be recovered from \cref{eq:hcnn} by taking only $h=2^n$ for integer $n$.
...
\end{equation}
and for any $f_k$, its $h$ harmonic will be located at $f_{k + B}$.
...
The harmonic CNN is somewhat more difficult to implement, since the $h$th harmonic of a frequency $f_k$ will not generally coincide with an integer bin number, since this would require
\begin{eqnarray}
    f_j = 2^{j/B} \cdot f_{\min} &=& h \cdot 2^{k/B} f_{\min} = h \cdot f_k \notag \\
    % 2^{j/B} = h * 2^{k/B}
    % j/B = log_2 h + k/B
    % j = k + B log_2 h
...
\begin{eqnarray}
    ...
    &=& h \cdot f^1_k.
\end{eqnarray}
This implies that for all frequency bins $f$, the $h$th harmonic of $X^1_{t, f}$ is located at $X^h_{t, f}$.
