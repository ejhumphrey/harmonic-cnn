...
We computed log-magnitude CQT spectra from recordings in the USPop dataset~\cite{berenzweig2004large}, and drew 25K windowed observations uniformly at random.
...
\Cref{fig:corr} (right) demonstrates there are two notable differences from \Cref{fig:corr} (left).
...
Second, these correlations are not symmetric in frequency: aside from the weak correlation ath the second subharmonic (-1), correlations tend to be stronger in the positive harmonics.
